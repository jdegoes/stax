...
All collection are immutable, support arbitrary element types, and are unifed via a <code>Collection</code> interface, which provides access to functionality that is universally available across all kinds of collections (principally, <code>add()</code>, <code>addAll()</code>, <code>remove()</code>, <code>removeAll()</code>, <code>size</code>,  and <code>contains()</code>).

Collections implement the <code>Foldable</code> interface, so it's possible to gain access to much advanced functionality by using the <code>Foldable</code> extensions:
...
You can import this representation and helper functions with the following line of code:
...
This line imports a <code>Env</code> class that provides access to top-level JavaScript objects and functions.  <code>Env</code> automatically imports <code>Dom</code> which contains the external declarations that inform HaXe how every element in the DOM is typed.
